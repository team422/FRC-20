...
public class RpLidarLowLevelDriver {
	 ...
	 * @throws Exception
	 */
	public RpLidarLowLevelDriver(String portName, RpLidarListener listener) {
		System.out.println("Opening port " + portName);

		this.listener = listener;
		serialPort = new SerialPort(115200, SerialPort.Port.kUSB,8,SerialPort.Parity.kNone,SerialPort.StopBits.kOne);
	...
	protected void sendPayLoad(byte command, byte[] payLoad) {
		...
		dataOut[3 + payLoad.length] = (byte) checksum;
			...
			outByte = dataOut;
			serialPort.write(outByte, 3+payLoad.length + 1);
			serialPort.flush();
