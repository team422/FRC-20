    ...
    public AutonomousSwitch(StartingPosition StartingPosition, double Delay, boolean PushRobot, IntakeSource IntakeSource, boolean VisionEnabled) {
        ...
        if (startingPosition == AutonomousSwitch.StartingPosition.CENTER) {
            ...
            if (IntakeSource == AutonomousSwitch.IntakeSource.TRENCH) {
                ...
                addSequential(new Turn(180 - Math.toDegrees(Math.atan(66.91/(86.63-(robotLength/2)))), 0.3, 8)); //turn left towards trench
                ...
                addSequential(new Turn(Math.toDegrees(Math.atan(66.91/(86.63-(robotLength/2)))), 0.3, 8)); //turn (same amount) right towards trench
                ...
                addSequential(new DriveStraight(115*Math.tan(Math.toDegrees(22.5))*Math.cos(Math.toDegrees(22.5)), 0.5, 8)); //go to intake those 2 cells & cross 2x4
                addSequential(new Turn(-90, 0.3, 8)); //turn towards 3 remaining cells
                //addSequential(new IntakeOff()); //turn intake off & down
                //addSequential(new IntakeRetract());
                ...
                addSequential(new DriveStraight(90, 0.5, 8)); //go to intake last of 3 cells
                addSequential(new Turn(-90, 0.3, 8)); //turn so shooter faces towards power port
                addSequential(new DriveStraight(30, 0.5, 8)); //go to intake last of 3 cells
                addSequential(new Turn(-90, 0.3, 8)); //turn so shooter faces towards power port
                //addSequential(new IntakeOff()); //turn intake off & up
                //????
                addSequential(new DriveStraight(80, 0.5, 8)); //go to intake last of 3 cells
                addSequential(new Turn(90, 0.3, 8)); //turn so shooter faces towards power port
